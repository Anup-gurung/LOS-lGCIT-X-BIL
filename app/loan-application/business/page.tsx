...
function BusinessLoanApplicationContent() {
  ...
  // Slider states (kept for consistency)
  ...
  const isSameString = (a: string, b: string) =>
  ...
  // ---------- Pre‑fill loan type from session ----------
  useEffect(() => {
    ...
    const index = loanTypeOptions.findIndex(opt =>
      isSameString(opt.loan_type, loanTypeString)
    );
    if (index !== -1) {
  ...
  // ---------- Load loan sectors when loan type changes ----------
  useEffect(() => {
    if (!selectedLoanType) {
      setLoanSectorOptions([]);
      return;
    }
    const index = parseInt(selectedLoanType.split("-")[1]);
    if (isNaN(index) || !loanTypeOptions[index]) return;
    ...
    const loadSectors = async () => {
      ...
      try {
        ...
        //   ALLOWED_SECTORS.some(allowed =>
        //     isSameString(allowed, sector.loan_sector)
        //   )
        // );
    ...
    loadSectors();
  }, [selectedLoanType, loanTypeOptions]);

  // ---------- Pre‑fill sector from session ----------
  useEffect(() => {
    if (!sessionData) return;
    if (!selectedLoanType) return;
    ...
    const option = loanSectorOptions.find(opt =>
      isSameString(opt.loan_sector, sectorString)
    );
    if (option) {
  ...
  // ---------- Load loan sub‑sectors when sector changes ----------
  useEffect(() => {
    if (!selectedSector) {
    ...
    const loadSubSectors = async () => {
      ...
      try {
        const subSectors = await fetchLoanSubSectors(selectedSector);
  ...
  // ---------- Pre‑fill sub‑sector from session ----------
  useEffect(() => {
    if (!sessionData) return;
    if (!selectedSector) return;
    ...
    const index = loanSubSectorOptions.findIndex(opt =>
      isSameString(opt.sub_sector, subSectorString)
    );
    if (index !== -1) {
  ...
  // ---------- Load categories when sub‑sector changes ----------
  useEffect(() => {
    ...
    const subSectorId = selectedSubSector.split("-")[0];
  ...
  // ---------- Pre‑fill category from session ----------
  useEffect(() => {
    if (!sessionData) return;
    if (!selectedSubSector) return;
    ...
    const index = subSectorCategoryOptions.findIndex(opt =>
      isSameString(opt.sub_cat_sector, categoryString)
    );
    if (index !== -1) {
  ...
  // ---------- Set tenure/rate from selected sub‑sector (or category if available) ----------
  useEffect(() => {
    if (!selectedSubSector) {
      setApiTenure(0);
      setApiInterestRate(0);
    ...
    const index = parseInt(selectedSubSector.split("-")[1]);
  ...
  // ---------- Pre‑fill years after apiTenure is ready ----------
  useEffect(() => {
    if (!sessionData) return;
    if (hasSetYearsRef.current) return;
    if (apiTenure === 0) return;
  ...
  // ---------- Helper to get string values from IDs ----------
